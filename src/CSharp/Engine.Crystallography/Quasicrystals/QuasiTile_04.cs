...
namespace Leap71.Engine.Crystallography
{
    using ShapeKernel;
	...
	namespace AperiodicTiling
    {
        ...
        public class QuasiTile_04 : QuasiTile
        {
            /// <summary>
            /// Fourth type of rhombic 3D quasi-tile.
            /// </summary>
            public QuasiTile_04(LocalFrame oFrame0c, float fFaceSide = 20f)
            {
                m_clr                       = Cp.clrBlueberry;
                ...
                float fTileSide             = (sRefFace.vecPt2 - sRefFace.vecPt1).Length();
                ...
                for (int i = 0; i < 5; i++)
                {
                    ...
                    IcosehedralFace sFace1b = new IcosehedralFace(oFrame1b, EDef.LONG_AXIS, EConnector.LINE, fFaceSide);
                ...
                float fMaxZ         = fLowerZ + fUpperZ + fTileSide;
                ...
                // middle faces
                List<IcosehedralFace> aMiddleFaces = new List<IcosehedralFace>();
                for (int i = 0; i < 5; i++)
                {
                    {
                        IcosehedralFace sFace1u = aUpperSideFaces[i];
                        IcosehedralFace sFace1l = aLowerSideFaces[4 - i];
                        Vector3 vecLong1m       = (sFace1u.vecPt1 - sFace1l.vecPt1).Normalize();
                        Vector3 vecShort1m      = (sFace1u.vecPt2 - sFace1l.vecPt2).Normalize();
                        Vector3 vecNormal1m     = Vector3.Cross(vecShort1m, vecLong1m);
                        LocalFrame oFrame1m     = new LocalFrame(sFace1l.vecPt1, vecNormal1m, vecLong1m);

                        IcosehedralFace sFace1m = new IcosehedralFace(oFrame1m, EDef.LONG_AXIS, EConnector.LINE, fFaceSide);
                        aMiddleFaces.Add(sFace1m);
                    }
                    {
                        int iUpperIndex = i + 1;
                        if (iUpperIndex > 4)
                        ...
                        IcosehedralFace sFace1u = aUpperSideFaces[iUpperIndex];
                        IcosehedralFace sFace1l = aLowerSideFaces[4 - i];
                        Vector3 vecLong1m       = (sFace1u.vecPt1 - sFace1l.vecPt1).Normalize();

                        int iLowerIndex = 4 - i - 1;
                        if (iLowerIndex < 0)
                        {
                            iLowerIndex += 5;
                        }
                        IcosehedralFace sFace2u = aUpperSideFaces[i];
                        IcosehedralFace sFace2l = aLowerSideFaces[iLowerIndex];
                        Vector3 vecShort1m      = (sFace2u.vecPt2 - sFace2l.vecPt2).Normalize();
                        Vector3 vecNormal1m     = Vector3.Cross(vecShort1m, vecLong1m);
                        LocalFrame oFrame1m     = new LocalFrame(sFace1l.vecPt1, vecNormal1m, vecLong1m);
                ...
                m_aFaces.AddRange(aMiddleFaces);
            }
        }
	}
}